...
public:
    ...
    void ProcessKeyboard(Camera_Movement direction, float deltaTime)
    {
        ...
        glm::vec3 horizontalFront = glm::normalize(glm::vec3(Front.x, 0.0f, Front.z)); // Normalize horizontal component

        switch (direction)
        {
        case FORWARD:
            Position += (movementType == FREE) ? Front * velocity : horizontalFront * velocity;
            break;
        case BACKWARD:
            Position -= (movementType == FREE) ? Front * velocity : horizontalFront * velocity;
            break;
        case LEFT:
            Position -= (movementType == FREE) ? Right * velocity : glm::normalize(glm::vec3(Right.x, 0.0f, Right.z)) * velocity;
            break;
        case RIGHT:
            Position += (movementType == FREE) ? Right * velocity : glm::normalize(glm::vec3(Right.x, 0.0f, Right.z)) * velocity;
            break;
    ...
    void ProcessMouseMovement(float xoffset, float yoffset, GLboolean constrainPitch = true, float _dt = .0f)
    {
        ...
        Pitch = std::fmod((Pitch + yoffset), (GLfloat)360.0f);
        //Pitch += yoffset;
