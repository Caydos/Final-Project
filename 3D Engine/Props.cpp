...
std::vector<Map::Decor> Props::Generate(Map::Cell _cell)
{
	...
	{
		if (_cell.wallList[wallId].name == "../Sets/HOSPITAL/Map/HSP_Door.json" || _cell.wallList[wallId].name == "../Sets/HOSPITAL/Map/TinyDoor.json" && _cell.wallList[wallId].isVisible)
		{
			Map::Decor door;
...
std::vector<Map::Decor> Props::GeneratePropsForCell(Map::Cell& cell, int _random)
{
